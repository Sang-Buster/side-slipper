...
def update_plot(fig, df, selected_columns, plot_data, current_time_index):
    ...
    fig.add_vline(x=current_time, line_dash="dash", line_color="red", name="Current Time")
...
def display_multi_select_and_line_plot(df, current_time_index):
    ...
    plottable_columns = ['CoG_base', 'Lat_base', 'Lon_base', 'VX_base', 'VY_base', 'VZ_base',
                         'CoG_rover', 'Lat_rover', 'Lon_rover', 'VX_rover', 'VY_rover', 'VZ_rover',
                         'relPosHeading', 'beta']

    selected_columns = st.multiselect("Select data to visualize:", plottable_columns, default=['CoG_base', 'CoG_rover'])
    ...
    if selected_columns:
        ...
        if 'line_plot_fig' not in st.session_state or st.session_state.line_plot_selected_columns != selected_columns:
            st.session_state.line_plot_fig = create_initial_plot(df, selected_columns, plot_data)
            st.session_state.line_plot_selected_columns = selected_columns

        # Update the plot with current data
        fig = update_plot(st.session_state.line_plot_fig, df, selected_columns, plot_data, current_time_index)
