...
def display_map(df, current_time_index, initial_lat=29.18853467, initial_lon=-81.04548):
    ...
    if "Lat_base" not in df.columns or "Lon_base" not in df.columns:
        st.error("Could not find latitude and longitude columns after cleaning. Please check your CSV file.")
        return
    ...
    if current_time_index > 0:
        ...
        folium.PolyLine(
            base_coordinates, weight=2, color="blue", opacity=0.8, tooltip="Traveled Path",
            name='path'
        ).add_to(m)

    # Add or update the current position marker
    current_pos = [df["Lat_base"].iloc[current_time_index], df["Lon_base"].iloc[current_time_index]]
    folium.Marker(
        current_pos,
        popup="Current Position",
        icon=folium.Icon(color="blue", prefix='fa', icon="car"),
        name=f'marker_current'
    ).add_to(m)
